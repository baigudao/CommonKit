...
public class RefreshLayout extends ViewGroup {
    ...
    private boolean mIsBeginDraggedX;
...
//        imageView.setPadding(280,60,280,60);
//        imageView.setImageResource(R.drawable.bg_home_header);
    ...
    public void setRefreshHeader(View view) {
        if (view != null && view != refreshHeader) {
            ...
            if (layoutParams == null) {
                layoutParams = new LayoutParams(LayoutParams.MATCH_PARENT, dp2px(getContext(), 100));
        ...
        switch (actionMasked) {
            ...
            case MotionEvent.ACTION_MOVE:
                try {
                    ...
                    if (!mIsBeginDraggedX && Math.abs(x - initDownX) > Math.abs(y - initDownY)) {
                        mIsBeginDraggedX = true;
                    }
                    if (mIsBeginDraggedX) {
                        super.dispatchTouchEvent(ev);
                        return false;
                    ...
                    if (!mIsBeginDragged && Math.abs(y - initDownY) > Math.abs(x - initDownX)) {
